...
//            $this->sendResponse(401);
    ...
    {
        /* please check tools.js, how it gets the value of username and its token */
        /* it should be modified to have better credentials check */
        if (isset(Yii::app()->request->cookies['_kmodeling'])) {
            $cookie = Yii::app()->request->cookies['_kmodeling']->value;
            if (strpos($cookie, ',')) {
                list($username, $token) = explode(',', $cookie);
                if (md5($this->tempUsername . $this->tempPassword) == $token)
                    return true;
            }
